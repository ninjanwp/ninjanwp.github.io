...
const TEXT_COLOR = "text-zinc-50";
const HOVER_TEXT_COLOR = "text-custom-blue-400";
...
const ZoopText: React.FC<ZoopTextProps> = ({ children, IconComponent }) => {
  ...
  return (
    ...
    >
      <div className={`${TEXT_COLOR} ${TEXT_SIZE}`}>
        {IconComponent}
      </div>

      <motion.p className="overflow-hidden leading-tight hidden px-2 sm:block">
        {characters.map((char, index) => (
          ...
          >
            ...
            <motion.span
              className={`block absolute bottom-0 ${HOVER_TEXT_COLOR} uppercase ${char === "→" ? "flex items-center" : ""}`}
              variants={{
                rest: {
                  y: "100%",
                  transition: {
